...
class ExpandableListAdapterEmpresas(var context: Context, var expandableListView : ExpandableListView, var header : MutableList<String>, var body : MutableList<MutableList<Long>>)  : BaseExpandableListAdapter() {
    ...
    override fun getChildView(groupPosition: Int, childPosition: Int, isLastChild: Boolean, convertView: View?, parent: ViewGroup?): View? {
        ...
        if(childPosition == 0){
            ...
            title?.setOnClickListener {
                ...
                var intent = Intent(context, DetallesEmpresa::class.java)
            ...
            title?.setOnClickListener {
                var id = getChild(groupPosition,childPosition)
                var intent = Intent(context, ModificarEmpresa::class.java)
                intent.putExtra("idEmpresa", id)
                context.startActivity(intent)
            }
    ...
    fun accionDeEliminar(id: Long) {
        ...
        mDialog.button_elimina.setOnClickListener {
            ...
            val call: Call<Cuenta> = cuentasService.deleteCuenta(id)

            call.enqueue(object: Callback<Cuenta> {
                override fun onResponse(call: Call<Cuenta>, response: Response<Cuenta>) {
                    if (response.isSuccessful()){
                        Toast.makeText(context,
                            "Empresa eliminada exitosamente", Toast.LENGTH_SHORT).show()
                        var intent = Intent(context, ListaDeEmpresas::class.java)
                        context.startActivity(intent)
                    }else{
                        Toast.makeText(context,
